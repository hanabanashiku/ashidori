...
export default class NetflixService {
    ...
    async getEpisodeMetadata(movieId) {
        const response = await axios.get(`${this.baseUrl}/metadata?movieid=${movieId}&imageFormat=webp&withSize=true&materialize=true`);
        const video = response.data.video;
        let currentSeason, currentEpisode;

        const episodeCount = video.seasons.reduce((previous, current) => previous + current.episodes.length, 0);
